...
def run_install [mountpoint: string] {
    # If USE_LOCAL_BOOTUPD is set, mount the local bootupd into the container
    # The prepare step copies it to /var/tmp/local-bootupd (since /usr is read-only)
    # Mount as both bootupd and bootupctl since bootupctl may be a separate binary in the container
    let bootupd_mount = if ($env.USE_LOCAL_BOOTUPD? == "1") {
        if ("/var/tmp/local-bootupd" | path exists) {
            print "Mounting local bootupd into container as bootupd and bootupctl"
            print $"Local bootupd help: (^/var/tmp/local-bootupd --help | lines | first)"
            ["-v" "/var/tmp/local-bootupd:/usr/bin/bootupd:ro" "-v" "/var/tmp/local-bootupd:/usr/bin/bootupctl:ro"]
        } else {
            print "WARNING: USE_LOCAL_BOOTUPD=1 but /var/tmp/local-bootupd not found on host"
            print "The TMT prepare step may not have run correctly. Using container's bootupd."
            []
        }
    } else {
        []
