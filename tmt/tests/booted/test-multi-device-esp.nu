...
# Prepare the system and run bootc install to-existing-root
def run_install [mountpoint: string] {
    bootc usr-overlay
    if ("/sysroot/ostree" | path exists) {
        mount --bind /usr/share/empty /sysroot/ostree
    }
    rm -rf /usr/lib/bootupd/updates
    rm -rf /usr/lib/bootc/bound-images.d

    bootc install to-existing-root --disable-selinux --source-imgref $target_image --acknowledge-destructive --target-no-signature-verification $mountpoint
}
