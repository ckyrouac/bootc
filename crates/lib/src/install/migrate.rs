...
///   `<root_path>/sysroot/ostree/deploy/<stateroot>/deploy/<hash>.0`
///
/// We enumerate `<root_path>/sysroot/ostree/deploy/` and return the most
/// recently modified entry under each stateroot's `deploy/` subdirectory.
#[context("Locating new ostree deployment directory")]
fn find_deploy_dir(root_path: &Path) -> Result<PathBuf> {
    let stateroot_base = root_path.join("sysroot/ostree/deploy");
...
fn preserve_var_reflink(src_var: &Path, new_var: &Path) -> Result<()> {
    ...
    for entry in entries {
        ...
        // For `log/`, skip only the `journal` subdirectory, not all of `log/`.
